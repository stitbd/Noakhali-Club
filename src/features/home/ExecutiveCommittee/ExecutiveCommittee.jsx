...
const COMMITTEE = [
  ...
  // img17 is available as a spare — assign to an 18th member if needed
];
...
const MemberCard = ({ member, index, cardRef }) => {
  ...
  return (
    <div
      ...
      className={`${styles.card} ${isPresident ? styles.presidentCard : ''}`}
    >
      {/* Photo area */}
      ...
      <div className={styles.body}>
        {isPresident && <p className={styles.presidentLabel}>Club President</p>}
...
const ExecutiveCommittee = () => {
  ...
  const rest      = COMMITTEE.filter(m => m.role !== 'President');
    ...
    <section className={styles.section}>
      <Container>
        ...
        <Row className="justify-content-center mb-5">
          <Col lg={7} md={10}>
            <MemberCard member={president} index={0} cardRef={cardRef} />
          </Col>
        </Row>
        ...
        <Row className="g-4 justify-content-center">
          {rest.map((member, i) => (
            <Col key={i} lg={4} md={6}>
              <MemberCard member={member} index={i + 1} cardRef={cardRef} />
            </Col>
          ))}
        </Row>
      </Container>
